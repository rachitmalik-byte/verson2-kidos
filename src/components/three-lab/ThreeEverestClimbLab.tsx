...
export const ThreeEverestClimbLab: React.FC<Props> = ({ onCompleted }) => {
  ...
  const handleAltitudeChange = (val: number) => {
    ...
    if (val >= 8800) {
      ...
      sounds.tensionSnap();
      voiceAssistant.speak(
        'Entering the Death Zone! At 7,900m altitude, atmospheric pressure is only 330 hPa. Oxygen cylinder is required!'
      );
    } else {
      sounds.pop();
    }
  ...
  useEffect(() => {
    ...
    scene.background = new THREE.Color(0x0f172a); // High Alpine Sky
    ...
    // 3. 3D Mount Everest Pyramid Peak & Ridge
    const mountainGroup = new THREE.Group();
    scene.add(mountainGroup);

    // Main Everest Peak Pyramid
    const peakGeo = new THREE.ConeGeometry(7, 8, 4);
    ...
    peakMesh.position.set(0, 2.2, -2.5);
    peakMesh.rotation.y = Math.PI / 4;
    mountainGroup.add(peakMesh);

    // Snow & Glacier Ice Cap on Peak
    const glacierGeo = new THREE.ConeGeometry(3.6, 4.2, 4);
    ...
    glacierMesh.position.set(0, 4.2, -2.5);
    glacierMesh.rotation.y = Math.PI / 4;
    mountainGroup.add(glacierMesh);

    // Climbing Ridge Slope (Catmull curve from base to summit)
    const ridgeCurve = new THREE.CatmullRomCurve3([
      new THREE.Vector3(-4.5, -1.2, 1.5), // 0m
      new THREE.Vector3(-2.2, 0.2, 0.8),  // 5,364m Base Camp
      new THREE.Vector3(0.0, 1.8, 0.0),   // 7,900m Death Zone
      new THREE.Vector3(0.8, 3.6, -1.2),  // 8,848m Summit
    ]);

    const ridgeGeo = new THREE.TubeGeometry(ridgeCurve, 32, 0.35, 12, false);
    ...
    flagMesh.position.set(1.15, 4.6, -1.2);
    mountainGroup.add(flagMesh);

    // 4. Detailed 3D Mountaineer Model (Climbing along Ridge)
    const climberGroup = new THREE.Group();
    scene.add(climberGroup);

    // Red Thermal Down Suit Body
    ...
    const tankGeo = new THREE.CylinderGeometry(0.12, 0.12, 0.6, 16);
    ...
    // 5. Blizzard Snow Particles in Death Zone
    const snowCount = 150;
    ...
    for (let i = 0; i < snowCount; i++) {
      ...
      snowVels.push(
        new THREE.Vector3(
          -4.0 - Math.random() * 4.0, // High altitude hurricane winds
          -1.0 - Math.random() * 1.5,
          (Math.random() - 0.5) * 2.0
        )
      );
    ...
    const animate = () => {
      ...
      if (deathZone) {
        ...
        snowMat.opacity = 0.9;
        scene.background.setHex(0x020617); // Dark thin atmosphere
      } else {
        oxygenTank.visible = false;
        snowMat.opacity = 0.3;
        scene.background.setHex(0x0284c7); // Clear blue valley sky
      }
    ...
    <div className="w-full max-w-4xl bg-white p-5 sm:p-7 rounded-[36px] border-4 border-amber-400 shadow-2xl flex flex-col items-center select-none font-sans text-slate-900">
      ...
      <div className="w-full flex flex-col gap-3">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 w-full">
          {[
            { label: 'Valley (0m)', alt: 0, icon: '🌿' },
            { label: 'Base Camp (5,364m)', alt: 5364, icon: '⛺' },
            { label: 'Death Zone (7,900m)', alt: 7900, icon: '⚠️' },
            { label: 'Summit (8,848m)', alt: 8848, icon: '🚩' },
          ].map((preset) => (
            <button
              key={preset.alt}
              onClick={() => handleAltitudeChange(preset.alt)}
              className={`p-2.5 rounded-xl font-black text-xs cursor-pointer transition-all flex items-center justify-center gap-1.5 shadow-xs ${
