...
/* ============================================================================
   1. 🏔️ CHANGTHANG 5,000M PASHMINA THERMAL SIMULATOR (CHAPTER 1)
   ============================================================================ */
export const ChangthangPashminaSim: React.FC<{ onCompleted?: () => void }> = ({ onCompleted }) => {
  ...
  // Fleece thickness grows from 4px at +10°C to 20px at -40°C
  const fleeceThickness = Math.round(4 + ((10 - tempCelsius) / 50) * 16);
  const isFreezing = tempCelsius <= -20;
    ...
    <div className="w-full bg-white p-6 sm:p-8 rounded-3xl border-4 border-sky-400 shadow-xl flex flex-col items-center">
      ...
      <div className="w-full flex items-center justify-between mb-4 pb-3 border-b-2 border-slate-100 flex-wrap gap-2">
        <div className="flex items-center gap-2">
          ...
          <span className="text-xs font-black uppercase text-slate-800">
            Changthang Plateau (5,000m Cold Desert)
          </span>
        </div>
        <div className="flex items-center gap-2">
          <button
            onClick={() => {
              sounds.pop();
              setIsThermalVision(!isThermalVision);
            }}
            className={`px-3 py-1 rounded-full text-xs font-black border cursor-pointer transition-all ${
              isThermalVision
                ? 'bg-rose-500 text-white border-rose-600 shadow-md ring-2 ring-rose-300'
                : 'bg-slate-100 text-slate-700 border-slate-300'
            }`}
          >
            🔥 {isThermalVision ? 'Thermal Heat-Map ON' : 'Thermal Heat-Map OFF'}
          </button>
          <span className="text-xs font-black px-3.5 py-1 rounded-full bg-sky-100 text-sky-900 border border-sky-300">
            ❄️ TEMP: {tempCelsius}°C
          </span>
        </div>
      </div>

      {/* Dynamic Alpine Environment & Pashmina Goat Stage */}
      <div
        className={`relative w-full h-80 rounded-3xl overflow-hidden border-3 shadow-inner flex flex-col items-center justify-between p-4 transition-colors duration-700 ${
          isThermalVision
            ? 'bg-gradient-to-b from-blue-950 via-indigo-950 to-slate-950 border-purple-500'
            : tempCelsius <= -20
            ? 'bg-gradient-to-b from-sky-400 via-sky-200 to-slate-100 border-sky-400'
            : 'bg-gradient-to-b from-sky-300 via-amber-100 to-emerald-100 border-amber-300'
        }`}
      >
        {/* Snow Blizzard Effect at Sub-zero */}
        {tempCelsius <= -10 && !isThermalVision && (
          <div className="absolute inset-0 pointer-events-none">
            {[...Array(12)].map((_, i) => (
              <motion.div
                key={i}
                animate={{ y: [0, 300], x: [0, -40] }}
                transition={{ repeat: Infinity, duration: 1.5 + (i % 3) * 0.5, ease: 'linear' }}
                className="absolute text-white text-xs select-none opacity-70"
                style={{ left: `${(i * 9) % 100}%`, top: '-10px' }}
              >
                ❄️
              </motion.div>
            ))}
        ...
        {/* High Himalayan Peaks Backdrop */}
        <svg className="w-full h-36 absolute top-0 left-0 pointer-events-none opacity-80" viewBox="0 0 400 150">
          <polygon points="0,150 90,40 180,150" fill={isThermalVision ? '#1e1b4b' : '#ffffff'} stroke="#cbd5e1" />
          <polygon points="120,150 220,20 320,150" fill={isThermalVision ? '#1e1b4b' : '#ffffff'} stroke="#cbd5e1" />
          <polygon points="260,150 340,50 400,150" fill={isThermalVision ? '#1e1b4b' : '#ffffff'} stroke="#cbd5e1" />
        </svg>

        {/* Dynamic Rebo Yak-Hair Tent */}
        <div className="absolute bottom-6 left-6 z-10 flex flex-col items-center">
          <svg width="100" height="70" viewBox="0 0 100 70">
            <polygon points="10,65 50,15 90,65" fill="#1e293b" stroke="#0f172a" strokeWidth="2" />
            <line x1="50" y1="8" x2="50" y2="15" stroke="#78350f" strokeWidth="2" />
            <polygon points="42,65 50,42 58,65" fill="#020617" />
          </svg>
          <span className="text-[9px] font-black text-slate-800 bg-white/90 px-1.5 py-0.5 rounded-md shadow-xs">
            Yak-Hair Rebo Tent
          </span>
        </div>

        {/* Dynamic Pashmina Goat with Real-time Growing Fleece! */}
        <div className="my-auto z-10 flex flex-col items-center">
          <svg width="220" height="150" viewBox="0 0 220 150">
            ...
            <ellipse
              cx="110"
              cy="80"
              rx={35 + fleeceThickness}
              ry={25 + fleeceThickness}
              fill={
                isThermalVision
                  ? '#ef4444' // Trapped 38°C core heat
                  : '#ffffff'
              }
              stroke={isThermalVision ? '#f97316' : '#cbd5e1'}
              strokeWidth={fleeceThickness > 10 ? 4 : 2}
            />

            {/* Fluffy Cashmere Fleece Tufts (Scale with cold!) */}
            {[
              { cx: 85, cy: 75, r: 8 + fleeceThickness * 0.4 },
              { cx: 110, cy: 70, r: 10 + fleeceThickness * 0.5 },
              { cx: 135, cy: 75, r: 8 + fleeceThickness * 0.4 },
              { cx: 100, cy: 92, r: 9 + fleeceThickness * 0.4 },
              { cx: 125, cy: 92, r: 9 + fleeceThickness * 0.4 },
            ].map((c, i) => (
              <circle
                key={i}
                cx={c.cx}
                cy={c.cy}
                r={c.r}
                fill={isThermalVision ? '#f97316' : '#f8fafc'}
                stroke={isThermalVision ? '#fbbf24' : '#e2e8f0'}
              />
            ))}

            {/* Legs */}
            <line x1="85" y1="105" x2="85" y2="135" stroke={isThermalVision ? '#3b82f6' : '#475569'} strokeWidth="4" strokeLinecap="round" />
            <line x1="98" y1="105" x2="98" y2="135" stroke={isThermalVision ? '#3b82f6' : '#475569'} strokeWidth="4" strokeLinecap="round" />
            <line x1="122" y1="105" x2="122" y2="135" stroke={isThermalVision ? '#3b82f6' : '#475569'} strokeWidth="4" strokeLinecap="round" />
            <line x1="135" y1="105" x2="135" y2="135" stroke={isThermalVision ? '#3b82f6' : '#475569'} strokeWidth="4" strokeLinecap="round" />

            {/* Head */}
            <ellipse cx="155" cy="65" rx="15" ry="12" fill={isThermalVision ? '#fbbf24' : '#ffffff'} stroke="#cbd5e1" strokeWidth="2" />
            <circle cx="160" cy="62" r="2.5" fill="#0f172a" />
            {/* Twisted Horns */}
            <path d="M 148 55 Q 140 35 125 40" fill="none" stroke="#78350f" strokeWidth="3.5" strokeLinecap="round" />
            <path d="M 154 52 Q 148 30 135 34" fill="none" stroke="#78350f" strokeWidth="3.5" strokeLinecap="round" />
          </svg>
          ...
          >
            {isThermalVision
              ? '🔥 Core Body Temp: 38°C (Locked Inside Fleece!)'
              : `Fleece Insulation Thickness: ${fleeceThickness}mm (Extra Warmth)`}
          </span>
        </div>
      </div>
      ...
      <div className="w-full max-w-lg my-4 flex flex-col gap-2">
        <div className="flex justify-between items-center text-xs font-black text-slate-700">
          <span className="text-sky-700">❄️ Winter Deep Freeze (-40°C)</span>
          <span className="text-sky-600 font-black text-sm bg-sky-100 px-3 py-1 rounded-full">{tempCelsius}°C</span>
          <span className="text-amber-700">☀️ Summer (10°C)</span>
        </div>
        <input
          type="range"
          min="-40"
          ...
          className="w-full h-4 bg-sky-200 rounded-lg appearance-none cursor-pointer accent-sky-500"
        />
      </div>

      {/* Dynamic Explanation */}
      <div className="w-full bg-sky-50 p-4 rounded-2xl border-2 border-sky-200 text-center text-xs font-bold text-sky-950">
        🧤 <strong>Changpa Bio-Thermal Law:</strong> The colder the high mountain winter gets (-40°C), the finer and denser the goat grows its cashmere fleece to trap body heat! 1 Pashmina shawl = 6 thick sweaters!
      </div>
    </div>
  );
};
