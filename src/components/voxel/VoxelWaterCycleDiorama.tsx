...
import { Sun, CloudRain, Wind, Play, RotateCcw, Sparkles, AlertCircle } from 'lucide-react';
...
export const VoxelWaterCycleDiorama: React.FC<{ onComplete?: () => void }> = ({ onComplete }) => {
    ...
    try {
      ...
      for (let x = -7; x <= 7; x++) {
        for (let z = -7; z <= 7; z++) {
          ...
          if (x < 0 && dist < 6) {
            const mtnHeight = Math.floor((6 - dist) * 1.5 + (x < -2 ? 2 : 0));
            for (let y = 0; y <= mtnHeight; y++) {
              if (y >= 4) voxels.push({ x, y, z, color: 0xffffff });
              else if (y >= 2) voxels.push({ x, y, z, color: 0x7a7f85 });
              else voxels.push({ x, y, z, color: 0x4d8c2f });
            }
          } else if (x >= 0 && x <= 2 && z >= -2 && z <= 2) {
            voxels.push({ x, y: 0, z, color: 0x228be6, transparent: true, opacity: 0.85 });
          } else if (x > 3 && dist > 4.5) {
      ...
      for (let cx = -2; cx <= 2; cx++) {
        for (let cz = -2; cz <= 2; cz++) {
          if (Math.abs(cx) + Math.abs(cz) <= 3) {
            cloudVoxels.push({ x: cx, y: 0, z: cz, color: 0xffffff });
            cloudVoxels.push({ x: cx, y: 1, z: cz, color: 0xffffff });
          }
        }
      }
      ...
      const animate = () => {
        ...
        try {
          ...
          if (cloudGroupRef.current) {
            cloudGroupRef.current.position.y = 8 + Math.sin(t * 1.5) * 0.4;
          }
          ...
          console.warn('Animation loop warn:', renderErr);
        }
      };
  ...
  if (!hasWebGLSupport) {
    return (
      <div className="w-full p-6 bg-slate-900 rounded-3xl border-2 border-sky-400 text-center text-white">
        <AlertCircle className="w-8 h-8 text-amber-400 mx-auto mb-2" />
        <p className="text-sm font-bold">3D WebGL is optimizing. You can use the full 2D interactive lab below!</p>
      </div>
    );
  }
