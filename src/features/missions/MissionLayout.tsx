...
export const MissionLayout: React.FC<MissionLayoutProps> = ({
  ...
  return (
    <div className="min-h-screen w-full bg-[#F8FAFC] text-slate-900 flex flex-col justify-between pt-2 sm:pt-3 pb-20 px-3 sm:px-6 md:px-8 font-sans relative overflow-x-hidden selection:bg-teal-500 selection:text-white">
      ...
      <div className="w-full max-w-5xl mx-auto flex flex-col gap-4 relative z-10">
        ...
        <header className="w-full max-w-4xl mx-auto bg-white/92 backdrop-blur-xl border border-slate-200/90 px-3.5 sm:px-4 py-2 rounded-2xl shadow-xs flex flex-col gap-1.5 transition-all">
          <div className="flex items-center justify-between gap-3">
            {/* Navigation & Mission Info */}
            <div className="flex items-center gap-2 min-w-0">
              <button
                onClick={handleHomeClick}
                className="p-1.5 rounded-xl bg-slate-100 hover:bg-slate-200/80 text-slate-700 border border-slate-200/80 cursor-pointer transition-all"
              ...
              <button
                ...
                className="px-2.5 py-1 rounded-xl bg-slate-100 hover:bg-slate-200/80 text-slate-700 border border-slate-200/80 text-xs font-bold flex items-center gap-1.5 cursor-pointer transition-all"
              ...
              <div className="min-w-0 truncate ml-1 flex items-center gap-2">
                <span className="px-2 py-0.5 rounded-md bg-teal-50 text-teal-800 text-[11px] font-mono font-bold border border-teal-200/70">
                  M0{derivedNumber}
                </span>
      ...
      <footer className="fixed bottom-3 inset-x-0 mx-auto max-w-2xl z-40 px-3 pointer-events-none">
        <div className="pointer-events-auto bg-white/95 backdrop-blur-xl border border-slate-200/90 px-4 py-2 rounded-2xl shadow-lg shadow-slate-300/40 flex items-center justify-between gap-3 transition-all">
          <button
            onClick={() => {
              sounds.pop();
              onPrev();
            ...
            className="px-3 py-1.5 rounded-xl bg-slate-100 hover:bg-slate-200/80 text-slate-700 border border-slate-200/80 text-xs font-bold flex items-center gap-1.5 disabled:opacity-30 disabled:cursor-not-allowed cursor-pointer transition-all"
          >
          ...
          <span className="text-[11px] font-mono font-bold text-slate-600 bg-slate-100/90 px-3 py-1 rounded-full border border-slate-200/70">
          ...
          <button
            ...
            className={`px-5 py-2 rounded-xl font-bold text-xs flex items-center gap-1.5 transition-all cursor-pointer ${
              isStepComplete
                ? 'bg-teal-600 hover:bg-teal-700 text-white shadow-xs shadow-teal-700/20'
